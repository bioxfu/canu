...
for (n in 1:length(bt_lst)) {
  m <- bt_lst[[n]]
  plot(0, xlim = c(0, max(m[7:10])), ylim = c(0, 10), type = 'n', yaxt='n', ylab='', xlab='', main = paste0('alignment length > ', length_cutoff, ' bp'))
  for(i in 1:nrow(m)) {
    if (m[i, 4] > length_cutoff) {
